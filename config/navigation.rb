...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    ...
    n.item :profile, safe_join([material_symbol('person'), t('settings.profile')]), settings_profile_path, if: -> { current_user.functional? && !self_destruct }, highlights_on: %r{/settings/profile|/settings/verification|/settings/privacy}
    ...
    n.item :moderation, safe_join([material_symbol('gavel'), t('moderation.title')]), nil, if: -> { current_user.can?(:manage_reports, :view_audit_log, :manage_users, :manage_invites, :manage_taxonomies, :manage_federation, :manage_blocks) && !self_destruct } do |s|
      ...
      s.item :direct_messages, safe_join([material_symbol('mail'), t('admin.direct_messages.title')]), admin_direct_messages_path, highlights_on: %r{/admin/direct_messages}, if: -> { current_user.can?(:administrator) }
